...
This script requires 'regex' for lookaround functionality not present in 're'.''',
    ...
    parser.add_argument("-d", "--xcode", required = False, action="store_true",
...
def render_entry_function(xcoder):
    return f"""
bool cbor_{xcoder.xcode_func_name()}(const uint8_t * p_payload, size_t payload_len, {xcoder.type_name()} * p_result)
{{
	cbor_state_t state = {{
		.p_payload = p_payload,
...
{(linesep+linesep).join([f"bool cbor_{xcoder.xcode_func_name()}(const uint8_t * p_payload, size_t payload_len, {xcoder.type_name()} * p_result);" for xcoder in entry_types])}
...
def main():
    ...
    mode = "decode" if args.xcode else "encode"

    if args.xcode == args.encode:
        args.error("Please specify exactly one of --xcode or --encode.")
