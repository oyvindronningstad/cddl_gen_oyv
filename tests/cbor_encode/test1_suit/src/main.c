...
bool cbor_encode_SUIT_Outer_Wrapper(const uint8_t * p_payload, size_t payload_len, SUIT_Outer_Wrapper_t * p_result);

bool cbor_encode_SUIT_Command_Sequence(const uint8_t * p_payload, size_t payload_len, SUIT_Command_Sequence_t * p_result);

bool cbor_encode_SUIT_Authentication_Wrapper(const uint8_t * p_payload, size_t payload_len, SUIT_Authentication_Wrapper_t * p_result);
...
{
	SUIT_Command_Sequence_t sequence1 = {0};
...
void test_manifest(const u8_t *input, size_t len)
{
	...
	cbor_print("test_vector end at: 0x%x\r\n",
				((uint32_t)input) + len);
	res = cbor_decode_SUIT_Outer_Wrapper(input, len, &outerwrapper1);
